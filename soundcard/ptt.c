 ...
 *
 * Support for Raspverry PI GPIO control of PTT by Michał Łaszczewski April 2020
 */
...
#endif
#ifdef HAVE_GPIOD_H
        { "rpi", "rPI", "Raspberry PI GPIO pin number", "0", MODEMPAR_COMBO, 
          { c: { { "00", "01", "02", "03", "04", "05", "06", "07", "08", "09", "10", "11", "12", "13", "14", "15", "16", "17",
...
#define PAR_RPIPIN (1+CM108GPIOPARAMS)
#ifdef HAVE_GPIOD_H
	const char* rpi_pin = params[PAR_RPIPIN];
	if(rpi_pin) {
		state->mode = rpi;
		state->gpio = strtoul(rpi_pin, NULL, 0);
		logprintf(MLOG_INFO, "Using Raspberry PI GPIO port %d\n", state->gpio);
		state->chip = gpiod_chip_open_by_name(path);
		if(!state->chip) {
			logprintf(MLOG_ERROR, "Open chip failed\n");
                        return -1;
		}
		state->line = gpiod_chip_get_line(state->chip, state->gpio);
		if (!state->line) {
			logprintf(MLOG_ERROR, "Get line failed\n");
			gpiod_chip_close(state->chip);
			return -1;
		}
		ret = gpiod_line_request_output(state->line, "Consumer", 0);
		if (ret < 0) {
			logprintf(MLOG_ERROR, "Request line as output failed\n");
			gpiod_line_release(state->line);
			gpiod_chip_close(state->chip);
			return -1;
		}
		ret = gpiod_line_set_value(state->line, 1);
		if (ret < 0) {
			logprintf(MLOG_ERROR, "Set line output failed\n");
			gpiod_line_release(state->line);
                        gpiod_chip_close(state->chip);
                        return -1;
		}
...
#define PAR_HAMLIBMODEL  (1+CM108GPIOPARAMS+RPIPARAMS)
#define PAR_HAMLIBPARAMS (2+CM108GPIOPARAMS+RPIPARAMS)
#ifdef HAVE_LIBHAMLIB
	const char *hamlib_model = params[PAR_HAMLIBMODEL];
	if ( state->mode == noport && hamlib_model && hamlib_model[0]) {
...
#ifdef HAVE_GPIOD_H
	case rpi: 
                logprintf(MLOG_INFO, "Set GPIO PTT pin %d to %d\n", state->gpio, state->ptt ? 0 : 1);
		ret = gpiod_line_set_value(state->line, state->ptt ? 0 : 1);
                if (ret < 0) {
                       logprintf(MLOG_ERROR, "Set line output failed\n");
                }
		return;
